...
@testingRouter.callback_query(
    TestingStates.information_before_test
    )
...
async def testing_cycle(poll_answer: PollAnswer, state: FSMContext):
    ...
    user_answers = {}
    for answer_id in poll_answer.option_ids:
    ...
    if question_num == len(test_data.questions): # Check end of the test
        ...
        await poll_answer.bot.send_message(chat_id=poll_answer.user.id, text=test_ending_txt)
